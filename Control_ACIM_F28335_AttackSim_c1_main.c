...
//==============================================================================
// Attack Simulation Setups
volatile Uint16 attackinitFlag = 0;  // attack flag
volatile Uint16 attackhaltFlag = 0;  // attack flag

Uint16 attackTicker = 0;
Uint16 attackPeriod = 10000;     // attack period fs=10kHz -> 1s
Uint16 recoverPeriod = 10000;    // recover period fs=10kHz -> 1s

_iq alpha = _IQ(0.025);    // attack coefficient
_iq impact = 0;           // impact factor
_iq offsetA_prev = 0;     // save the original offsetA for recover use
_iq offsetB_prev = 0;     // save the original offsetA for recover use
_iq offsetC_prev = 0;     // save the original offsetA for recover use
_iq SpeedRef_atk = _IQ(0.333333333);
Uint16 tick_atk_spd = 0;
...
//        pwmdac1.MfuncC2 = clarke1.Bs;
    ...
    //==========================================================================
        ...
        if (attackhaltFlag==0)
        {
            if (attackinitFlag==1) {offsetA = offsetA_prev + alpha;}
            else if (attackinitFlag==2) {offsetB = offsetB_prev + alpha;}
            else if (attackinitFlag==3) {offsetC = offsetC_prev + alpha;}
            else if (attackinitFlag==4) {offsetA = offsetA_prev + alpha; offsetB = offsetB_prev + alpha;}
            else if (attackinitFlag==5) {offsetA = offsetA_prev + alpha; offsetC = offsetC_prev + alpha;}
            else if (attackinitFlag==6) {offsetB = offsetB_prev + alpha; offsetC = offsetC_prev + alpha;}
            else if (attackinitFlag==7) {
                tick_atk_spd = tick_atk_spd + 1;
                if (tick_atk_spd==5000) {SpeedRef_atk = SpeedRef + _IQ(0.05);}
                else if (tick_atk_spd==10000) {SpeedRef_atk = SpeedRef - _IQ(0.05); tick_atk_spd=0;}
            }
            else {
                offsetA = offsetA_prev;
                offsetB = offsetB_prev;
                offsetC = offsetC_prev;
                SpeedRef_atk = SpeedRef;
                tick_atk_spd = 0;
            }
        }
        else {
    ...
    if (IsrTicker>=10000 && IsrTicker<20000)
        {
            ...
            offsetA_prev = offsetA;
            offsetB_prev = offsetB;
            offsetC_prev = offsetC;
        }
